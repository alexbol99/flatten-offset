(function (global, factory) {
    ...
    (global = global || self, factory(global['polygon-offset'] = {}));
     ...
     * @type {{CW: number, CCW: number, NOT_ORIENTABLE: number}}
     */
    const ORIENTATION$1 = {CCW:-1, CW:1, NOT_ORIENTABLE: 0};

    const PIx2$1 = 2 * Math.PI;
    ...
    for (let c in Constants$1) {Flatten$1[c] = Constants$1[c];}
    ...
    class Matrix$1 {
         ...
         **/
        clone() {
            return new Matrix$1(this.a, this.b, this.c, this.d, this.tx, this.ty);
        };
        ...
        translate(...args) {
            let tx, ty;
            if (args.length == 1 && (args[0] instanceof Flatten$1.Vector)) {
                tx = args[0].x;
                ty = args[0].y;
            } else if (args.length == 2 && typeof (args[0]) == "number" && typeof (args[1]) == "number") {
                tx = args[0];
                ...
                throw Flatten$1.Errors.ILLEGAL_PARAMETERS;
            }
            return this.multiply(new Matrix$1(1, 0, 0, 1, tx, ty))
        };
        ...
        rotate(angle) {
            let cos = Math.cos(angle);
            let sin = Math.sin(angle);
            return this.multiply(new Matrix$1(cos, sin, -sin, cos, 0, 0));
        };
         ...
         */
        scale(sx, sy) {
            return this.multiply(new Matrix$1(sx, 0, 0, sy, 0, 0));
        };
    ...
    Flatten$1.Matrix = Matrix$1;
    /**
     * Function to create matrix equivalent to "new" constructor
     * @param args
     */
    const matrix$1 = (...args) => new Flatten$1.Matrix(...args);
    Flatten$1.matrix = matrix$1;
     ...
     * @type {Interval}
     */
    const Interval$1 = class Interval {
        /**
         * Accept two comparable values and creates new instance of interval
         * Predicate Interval.comparable_less(low, high) supposed to return true on these values
         * @param low
    ...
    const RB_TREE_COLOR_RED$1 = 0;
    const RB_TREE_COLOR_BLACK$1 = 1;
    ...
    class Node$1 {
        constructor(key = undefined, value = undefined,
                    left = null, right = null, parent = null, color = RB_TREE_COLOR_BLACK$1) {
            this.left = left;                     // reference to left child node
            this.right = right;                   // reference to right child node
            this.parent = parent;                 // reference to parent node
            this.color = color;
            ...
            if (key && key instanceof Array && key.length == 2) {
                if (!Number.isNaN(key[0]) && !Number.isNaN(key[1])) {
                    this.item.key = new Interval$1(Math.min(key[0], key[1]), Math.max(key[0], key[1]));
                }
            }
            this.max = this.item.key ? this.item.key.max : undefined;
        }
        ...
        isNil() {
            return (this.item.key === undefined && this.item.value === undefined &&
                this.left === null && this.right === null && this.color === RB_TREE_COLOR_BLACK$1);
        }
    ...
    class IntervalTree$1 {
        ...
        insert(key, value = key) {
            if (key === undefined) return;
            let insert_node = new Node$1(key, value, this.nil_node, this.nil_node, null, RB_TREE_COLOR_RED$1);
            this.tree_insert(insert_node);
            this.recalc_max(insert_node);
            return insert_node;
        }
         ...
         */
        exist(key, value) {
            let search_node = new Node$1(key, value);
            return this.tree_search(this.root, search_node) ? true : false;
        }
         ...
         */
        search(interval, outputMapperFn = (value, key) => value ? value : key.output()) {
            let search_node = new Node$1(interval);
            let resp_nodes = [];
            this.tree_search_interval(this.root, search_node, resp_nodes);
            return resp_nodes.map(node => outputMapperFn(node.item.value, node.item.key))
        }
    ...
    // Go upwords to the root and re-color until violation will be resolved
        insert_fixup(insert_node) {
            ...
            current_node = insert_node;
            while (current_node != this.root && current_node.parent.color == RB_TREE_COLOR_RED$1) {
                if (current_node.parent == current_node.parent.parent.left) {   // parent is left child of grandfather
                    uncle_node = current_node.parent.parent.right;              // right brother of parent
                    if (uncle_node.color == RB_TREE_COLOR_RED$1) {             // Case 1. Uncle is red
                        // re-color father and uncle into black
                        current_node.parent.color = RB_TREE_COLOR_BLACK$1;
                        uncle_node.color = RB_TREE_COLOR_BLACK$1;
                        current_node.parent.parent.color = RB_TREE_COLOR_RED$1;
                        current_node = current_node.parent.parent;
                    }
                    else {                                                    // Case 2 & 3. Uncle is black
                        if (current_node == current_node.parent.right) {     // Case 2. Current if right child
                        ...
                        current_node.parent.color = RB_TREE_COLOR_BLACK$1;    // Case 3. Current is left child.
                        // Re-color father and grandfather, rotate grandfather right
                        current_node.parent.parent.color = RB_TREE_COLOR_RED$1;
                        this.rotate_right(current_node.parent.parent);
                    }
                    ...
                    uncle_node = current_node.parent.parent.left;              // left brother of parent
                    if (uncle_node.color == RB_TREE_COLOR_RED$1) {             // Case 4. Uncle is red
                        // re-color father and uncle into black
                        current_node.parent.color = RB_TREE_COLOR_BLACK$1;
                        uncle_node.color = RB_TREE_COLOR_BLACK$1;
                        current_node.parent.parent.color = RB_TREE_COLOR_RED$1;
                        ...
                        current_node.parent.color = RB_TREE_COLOR_BLACK$1;    // Case 6. Current is right child.
                        // Re-color father and grandfather, rotate grandfather left
                        current_node.parent.parent.color = RB_TREE_COLOR_RED$1;
                        this.rotate_left(current_node.parent.parent);
                    }
            ...
            this.root.color = RB_TREE_COLOR_BLACK$1;
        }
        ...
        tree_delete(delete_node) {
            ...
            if (/*fix_node != this.nil_node && */cut_node.color == RB_TREE_COLOR_BLACK$1) {
                this.delete_fixup(fix_node);
            }
        }
        ...
        delete_fixup(fix_node) {
            ...
            while (current_node != this.root && current_node.parent != null && current_node.color == RB_TREE_COLOR_BLACK$1) {
                if (current_node == current_node.parent.left) {          // fix node is left child
                    brother_node = current_node.parent.right;
                    if (brother_node.color == RB_TREE_COLOR_RED$1) {   // Case 1. Brother is red
                        brother_node.color = RB_TREE_COLOR_BLACK$1;         // re-color brother
                        current_node.parent.color = RB_TREE_COLOR_RED$1;    // re-color father
                        this.rotate_left(current_node.parent);
                        brother_node = current_node.parent.right;                      // update brother
                    }
                    // Derive to cases 2..4: brother is black
                    if (brother_node.left.color == RB_TREE_COLOR_BLACK$1 &&
                        brother_node.right.color == RB_TREE_COLOR_BLACK$1) {  // case 2: both nephews black
                        brother_node.color = RB_TREE_COLOR_RED$1;              // re-color brother
                        current_node = current_node.parent;                  // continue iteration
                    }
                    else {
                        if (brother_node.right.color == RB_TREE_COLOR_BLACK$1) {   // case 3: left nephew red, right nephew black
                            brother_node.color = RB_TREE_COLOR_RED$1;          // re-color brother
                            brother_node.left.color = RB_TREE_COLOR_BLACK$1;   // re-color nephew
                            this.rotate_right(brother_node);
                            brother_node = current_node.parent.right;                     // update brother
                            // Derive to case 4: left nephew black, right nephew red
                        }
                        ...
                        brother_node.color = current_node.parent.color;
                        current_node.parent.color = RB_TREE_COLOR_BLACK$1;
                        brother_node.right.color = RB_TREE_COLOR_BLACK$1;
                        this.rotate_left(current_node.parent);
                        current_node = this.root;                         // exit from loop
                    }
                }
                    ...
                    // Go to cases 2..4
                    if (brother_node.left.color == RB_TREE_COLOR_BLACK$1 &&
                        brother_node.right.color == RB_TREE_COLOR_BLACK$1) {   // case 2
                        brother_node.color = RB_TREE_COLOR_RED$1;             // re-color brother
                        current_node = current_node.parent;                              // continue iteration
                    }
                    else {
                        if (brother_node.left.color == RB_TREE_COLOR_BLACK$1) {  // case 3: right nephew red, left nephew black
                            brother_node.color = RB_TREE_COLOR_RED$1;            // re-color brother
                            brother_node.right.color = RB_TREE_COLOR_BLACK$1;    // re-color nephew
                            this.rotate_left(brother_node);
                            brother_node = current_node.parent.left;                        // update brother
                            // Derive to case 4: right nephew black, left nephew red
                        }
                        ...
                        brother_node.color = current_node.parent.color;
                        current_node.parent.color = RB_TREE_COLOR_BLACK$1;
                        brother_node.left.color = RB_TREE_COLOR_BLACK$1;
                        this.rotate_right(current_node.parent);
                        current_node = this.root;                               // force exit from loop
                    }
                }
            ...
            current_node.color = RB_TREE_COLOR_BLACK$1;
        }
        ...
        testRedBlackProperty() {
            let res = true;
            this.tree_walk(this.root, function (node) {
                if (node.color == RB_TREE_COLOR_RED$1) {
                    if (!(node.left.color == RB_TREE_COLOR_BLACK$1 && node.right.color == RB_TREE_COLOR_BLACK$1)) {
                        res = false;
                    }
                }
            });
        ...
        testBlackHeightProperty(node) {
            ...
            let heightLeft = 0;
            let heightRight = 0;
            if (node.color == RB_TREE_COLOR_BLACK$1) {
                height++;
            }
            if (node.left != this.nil_node) {
                heightLeft = this.testBlackHeightProperty(node.left);
     ...
     * Set properties and methods
     */
    class PlanarSet$1 extends Set {
        /**
         * Create new empty instance of PlanarSet
         */
        constructor() {
            super();
            this.index = new IntervalTree$1();
        }
    ...
    Flatten$1.PlanarSet = PlanarSet$1;
    ...
    class Point$1 {
        ...
        rotate(angle, center = {x: 0, y: 0}) {
            ...
            return new Flatten$1.Point(x_rot, y_rot);
        }
        ...
        translate(...args) {
            if (args.length == 1 && (args[0] instanceof Flatten$1.Vector)) {
                return new Flatten$1.Point(this.x + args[0].x, this.y + args[0].y);
            }

            if (args.length == 2 && typeof (args[0]) == "number" && typeof (args[1]) == "number") {
                return new Flatten$1.Point(this.x + args[0], this.y + args[1]);
            }
     ...
     * @param args
     */
    const point$1 = (...args) => new Flatten$1.Point(...args);
    Flatten$1.point = point$1;
    ...
    class Vector$1 {
        ...
        normalize() {
            ...
            throw Flatten$1.Errors.ZERO_DIVISION;
        }
         ...
         */
        rotate(angle) {
            let point = new Flatten$1.Point(this.x, this.y);
            let rpoint = point.rotate(angle);
            return new Flatten$1.Vector(rpoint.x, rpoint.y);
        }
    ...
    class Segment$1 {
        ...
        intersect(shape) {
            ...
            if (shape instanceof Flatten$1.Line) {
                return Segment$1.intersectSegment2Line(this, shape);
            }

            if (shape instanceof Flatten$1.Segment) {
                return Segment$1.intersectSegment2Segment(this, shape);
            }

            if (shape instanceof Flatten$1.Circle) {
                return Segment$1.intersectSegment2Circle(this, shape);
            }

            if (shape instanceof Flatten$1.Arc) {
                return Segment$1.intersectSegment2Arc(this, shape);
            }

            if (shape instanceof Flatten$1.Polygon) {
                return Flatten$1.Polygon.intersectShape2Polygon(this, shape);
            }
        }
         ...
         */
        reverse() {
            return new Segment$1(this.end, this.start);
        }
         ...
         */
        translate(...args) {
            return new Segment$1(this.ps.translate(...args), this.pe.translate(...args));
        }
        ...
        transform(matrix = new Flatten$1.Matrix()) {
            return new Segment$1(this.ps.transform(matrix), this.pe.transform(matrix))
        }
    ...
    Flatten$1.Segment = Segment$1;
    /**
     * Shortcut method to create new segment
     */
    const segment$1 = (...args) => new Flatten$1.Segment(...args);
    Flatten$1.segment = segment$1;
    ...
    class Line$1 {
        ...
        constructor(...args) {
            ...
            if (args.length == 2) {
                ...
                if (a1 instanceof Flatten$1.Point && a2 instanceof Flatten$1.Point) {
                    this.pt = a1;
                    this.norm = Line$1.points2norm(a1, a2);
                    return;
                }
        ...
        intersect(shape) {
            ...
            if (shape instanceof Flatten$1.Line) {
                return Line$1.intersectLine2Line(this, shape);
            }

            if (shape instanceof Flatten$1.Circle) {
                return Line$1.intersectLine2Circle(this, shape);
            }
            ...
            if (shape instanceof Flatten$1.Arc) {
                return Line$1.intersectLine2Arc(this, shape);
            }

            if (shape instanceof Flatten$1.Polygon) {
                return Flatten$1.Polygon.intersectLine2Polygon(this, shape);
            }
         ...
         */
        svg(box, attrs = {}) {
            let ip = Line$1.intersectLine2Box(this, box);
            if (ip.length === 0)
                return "";
            let ps = ip[0];
            let pe = ip.length == 2 ? ip[1] : ip.find(pt => !pt.equalTo(ps));
        ...
        static intersectLine2Line(line1, line2) {
            ...
            if (!Flatten$1.Utils.EQ_0(det)) {
                let new_ip = new Flatten$1.Point(detX / det, detY / det);
                ip.push(new_ip);
            }
            return ip;
        }
        ...
        static intersectLine2Box(line, box) {
            let pts = [
                new Flatten$1.Point(box.xmin, box.ymin),
                new Flatten$1.Point(box.xmax, box.ymin),
                new Flatten$1.Point(box.xmax, box.ymax),
                new Flatten$1.Point(box.xmin, box.ymax)
            ];
            let segs = [
        ...
        static intersectLine2Arc(line, arc) {
            ...
            if (Line$1.intersectLine2Box(line, arc.box).length == 0) {
                return ip;
            }
     ...
     * @param args
     */
    const line$1 = (...args) => new Flatten$1.Line(...args);
    Flatten$1.line = line$1;
    ...
    class Circle$1 {
         ...
         */
        contains(pt) {
            return Flatten$1.Utils.LE(pt.distanceTo(this.center)[0], this.r);
        }
        ...
        intersect(shape) {
            ...
            if (shape instanceof Flatten$1.Circle) {
                return Circle$1.intersectCirle2Circle(this, shape);
            }
     ...
     * @param args
     */
    const circle$1 = (...args) => new Flatten$1.Circle(...args);
    Flatten$1.circle = circle$1;
    ...
    class Arc$1 {
        ...
        split(pt) {
            ...
            if (Flatten$1.Utils.EQ_0(this.sweep))
                return [this.clone()];
        ...
        intersect(shape) {
            ...
            if (shape instanceof Flatten$1.Circle) {
                return Flatten$1.Arc.intersectArc2Circle(this, shape);
            }
            if (shape instanceof Flatten$1.Segment) {
                return shape.intersect(this);
            }
            if (shape instanceof Flatten$1.Arc) {
                return Flatten$1.Arc.intersectArc2Arc(this, shape);
            }
            if (shape instanceof Flatten$1.Polygon) {
                return Flatten$1.Polygon.intersectShape2Polygon(this, shape);
            }
        ...
        transform(matrix = new Flatten$1.Matrix()) {
            ...
            let newEnd = this.end.transform(matrix);
            let newCenter = this.pc.transform(matrix);
            let arc = Flatten$1.Arc.arcSE(newCenter, newStart, newEnd, this.counterClockwise);
            return arc;
        }
     ...
     * @param args
     */
    const arc$1 = (...args) => new Flatten$1.Arc(...args);
    Flatten$1.arc = arc$1;
    ...
    class Box$1 {
         ...
         */
        clone() {
            return new Box$1(this.xmin, this.ymin, this.xmax, this.ymax);
        }
     ...
     * @returns {Box}
     */
    const box$1 = (...args) => new Flatten$1.Box(...args);
    Flatten$1.box = box$1;

    const ray_shoot$1 = function(polygon, point) {
        let contains = undefined;
        ...
        // 4. Sort intersection in x-ascending order
        intersections.sort((i1, i2) => {
            if (LT$1(i1.pt.x, i2.pt.x)) {
                return -1;
            }
            if (GT$1(i1.pt.x, i2.pt.x)) {
                return 1;
            }
            return 0;
        });
        ...
        for (let i = 0; i < intersections.length; i++) {
            ...
            if (intersection.pt.equalTo(intersection.edge.shape.start)) {
                ...
                if (intersection.edge.shape instanceof Flatten$1.Segment) {
                    ...
                    /* Check if ray does not touch the curve in the extremal (top or bottom) point */
                    let box = intersection.edge.shape.box;
                    if (!(EQ$1(intersection.pt.y, box.ymin) ||
                        EQ$1(intersection.pt.y, box.ymax))) {
                        counter++;
                    }
                }
            }
        ...
        // 6. Odd or even?
        contains = counter % 2 == 1 ? Flatten$1.INSIDE : Flatten$1.OUTSIDE;
    ...
    class Edge$1 {
        ...
        setInclusion(polygon) {
            ...
            if (this.bvStart === undefined) {
                this.bvStart = ray_shoot$1(polygon, this.start);
            }
            if (this.bvEnd === undefined) {
                this.bvEnd = ray_shoot$1(polygon, this.end);
            }
            /* At least one end outside - the whole edge outside */
            ...
            /* Both are boundary - check the middle point */
                ...
                let bvMiddle = ray_shoot$1(polygon, this.middle());
                this.bv = bvMiddle;
            }
            return this.bv;
        }
     ...
     * Class implements circular bidirectional linked list
     */
    class CircularLinkedList$1 extends LinkedList$1 {
        constructor(first, last) {
            super(first, last);
            this.setCircularLinks();
        }
     ...
     * } while (edge != face.first)
     */
    class Face$1 extends CircularLinkedList$1 {
        constructor(polygon, ...args) {
            super();            // construct empty list of edges
            /**
             * Reference to the first edge in face
            ...
            if (args.length == 1) {
                if (args[0] instanceof Array) {
                    ...
                    if (shapes.every((shape) => {
                        return shape instanceof Flatten$1.Point
                    })) {
                        let segments = Face$1.points2segments(shapes);
                        this.shapes2face(polygon.edges, segments);
                    } else if (shapes.every((shape) => {
                ...
                /* Create new face and copy edges into polygon.edges set */
                else if (args[0] instanceof Face$1) {
                    let face = args[0];
                    this.first = face.first;
                    this.last = face.last;
                    for (let edge of face) {
                ...
                /* Instantiate face from circle circle in CCW orientation */
                    ...
                    this.shapes2face(polygon.edges, [args[0].toArc(Flatten$1.CCW)]);
                }
                /* Instantiate face from a box in CCW orientation */
        ...
        orientation() {
            if (this._orientation === undefined) {
                ...
                if (Flatten$1.Utils.EQ_0(area)) {
                    this._orientation = Flatten$1.ORIENTATION.NOT_ORIENTABLE;
                } else if (Flatten$1.Utils.LT(area, 0)) {
                    this._orientation = Flatten$1.ORIENTATION.CCW;
                } else {
                    this._orientation = Flatten$1.ORIENTATION.CW;
                }
            }
            return this._orientation;
        }
         ...
         */
        isSimple(edges) {
            let ip = Face$1.getSelfIntersections(this, edges, true);
            return ip.length == 0;
        }
    ...
    class Ray$1 {
        ...
        constructor(...args) {
            ...
            if (args.length == 2 && typeof (args[0]) == "number" && typeof (args[1]) == "number") {
                this.pt = new Flatten$1.Point(args[0], args[1]);
                return;
            }
         ...
         */
        get norm() {
            return new Flatten$1.Vector(0, 1);
        }
        ...
        intersectRay2Segment(ray, segment) {
            ...
            let line = new Flatten$1.Line(ray.start, ray.norm);
            let ip_tmp = line.intersect(segment);

            for (let pt of ip_tmp) {
                if (Flatten$1.Utils.GE(pt.x, ray.start.x)) {
                    ip.push(pt);
                }
            }
    ...
    const ray$1 = (...args) => new Flatten$1.Ray(...args);
    Flatten$1.ray = ray$1;
    ...
    class Polygon$1 {
         ...
         */
        contains(point) {
            let rel = ray_shoot$1(this, point);
            return (rel == Flatten$1.INSIDE || rel == Flatten$1.BOUNDARY) ? true : false;
        }
        ...
        intersect(shape) {
            ...
            if (shape instanceof Flatten$1.Line) {
                return Polygon$1.intersectLine2Polygon(shape, this);
            }

            if (shape instanceof Flatten$1.Circle ||
                shape instanceof Flatten$1.Segment ||
                shape instanceof Flatten$1.Arc) {
                return Polygon$1.intersectShape2Polygon(shape, this);
            }

            if (shape instanceof Flatten$1.Polygon) {
                return Polygon$1.intersectPolygon2Polygon(shape, this);
            }
        }
        ...
        static intersectPolygon2Polygon(polygon1, polygon2) {
            ...
            for (let edge1 of polygon1.edges) {
                for (let pt of Polygon$1.intersectShape2Polygon(edge1.shape, polygon2)) {
                    ip.push(pt);
                }
            }
    ...
    class Distance$1 {
        ...
        static point2segment(pt, segment) {
            /* Degenerated case of zero-length segment */
            if (segment.start.equalTo(segment.end)) {
                return Distance$1.point2point(pt, segment.start);
            }
        ...
        static point2arc(pt, arc) {
            ...
            let dist_and_segment = [];
            let dist, shortest_segment;
            [dist, shortest_segment] = Distance$1.point2circle(pt, circle);
            if (shortest_segment.end.on(arc)) {
                dist_and_segment.push(Distance$1.point2circle(pt, circle));
            }
            dist_and_segment.push(Distance$1.point2point(pt, arc.start));
            dist_and_segment.push(Distance$1.point2point(pt, arc.end));

            Distance$1.sort(dist_and_segment);
        ...
        static segment2line(seg, line) {
            ...
            let dist_and_segment = [];
            dist_and_segment.push(Distance$1.point2line(seg.start, line));
            dist_and_segment.push(Distance$1.point2line(seg.end, line));
         ...
         */
        static segment2segment(seg1, seg2) {
            let ip = Flatten$1.Segment.intersectSegment2Segment(seg1, seg2);
            if (ip.length > 0) {
                return [0, new Flatten$1.Segment(ip[0], ip[0])];   // distance = 0, closest point is the first point
            }
            ...
            dist_and_segment.push(Distance$1.point2segment(seg2.start, seg1));
            dist_and_segment.push(Distance$1.point2segment(seg2.end, seg1));
            dist_and_segment.push(Distance$1.point2segment(seg1.start, seg2));
            dist_and_segment.push(Distance$1.point2segment(seg1.end, seg2));
        ...
        static segment2circle(seg, circle) {
            ...
            let [dist, shortest_segment] = Distance$1.point2line(circle.center, line);
            if (Flatten$1.Utils.GE(dist, circle.r) && shortest_segment.end.on(seg)) {
                return Distance$1.point2circle(shortest_segment.end, circle);
            }
            /* Case 3. Otherwise closest point is one of the end points of the segment */
            else {
                let [dist_from_start, shortest_segment_from_start] = Distance$1.point2circle(seg.start, circle);
                let [dist_from_end, shortest_segment_from_end] = Distance$1.point2circle(seg.end, circle);
        ...
        static segment2arc(seg, arc) {
             ...
             * projection point belongs to segment AND
               * distance from projection point to circle belongs to arc  =>
               * return this distance from projection to circle */
            let [dist_from_center, shortest_segment_from_center] = Distance$1.point2line(circle.center, line);
            if (Flatten$1.Utils.GE(dist_from_center, circle.r) && shortest_segment_from_center.end.on(seg)) {
                let [dist_from_projection, shortest_segment_from_projection] =
                    Distance$1.point2circle(shortest_segment_from_center.end, circle);
                if (shortest_segment_from_projection.end.on(arc)) {
                    return [dist_from_projection, shortest_segment_from_projection];
                }
            }
            /* Case 3. Otherwise closest point is one of the end points of the segment */
            let dist_and_segment = [];
            dist_and_segment.push(Distance$1.point2arc(seg.start, arc));
            dist_and_segment.push(Distance$1.point2arc(seg.end, arc));

            let dist_tmp, segment_tmp;
            [dist_tmp, segment_tmp] = Distance$1.point2segment(arc.start, seg);
            dist_and_segment.push([dist_tmp, segment_tmp.reverse()]);

            [dist_tmp, segment_tmp] = Distance$1.point2segment(arc.end, seg);
            dist_and_segment.push([dist_tmp, segment_tmp.reverse()]);
        ...
        static circle2circle(circle1, circle2) {
            ...
            if (circle1.center.equalTo(circle2.center)) {
                let arc1 = circle1.toArc();
                let arc2 = circle2.toArc();
                return Distance$1.point2point(arc1.start, arc2.start);
            } else {
                // Case 2. Not concentric circles
                ...
                dist_and_segment.push(Distance$1.point2point(ip1[0], ip2[0]));
                dist_and_segment.push(Distance$1.point2point(ip1[0], ip2[1]));
                dist_and_segment.push(Distance$1.point2point(ip1[1], ip2[0]));
                dist_and_segment.push(Distance$1.point2point(ip1[1], ip2[1]));
        ...
        static circle2line(circle, line) {
            ...
            let [dist_from_center, shortest_segment_from_center] = Distance$1.point2line(circle.center, line);
            let [dist, shortest_segment] = Distance$1.point2circle(shortest_segment_from_center.end, circle);
            shortest_segment = shortest_segment.reverse();
            return [dist, shortest_segment];
        }
        ...
        static arc2line(arc, line) {
            ...
            if (Flatten$1.Utils.GE(dist_from_center, circle.r)) {
                ...
                let dist_and_segment = [];
                dist_and_segment.push(Distance$1.point2line(arc.start, line));
                dist_and_segment.push(Distance$1.point2line(arc.end, line));
        ...
        static arc2circle(arc, circle2) {
            ...
            let [dist, shortest_segment] = Distance$1.circle2circle(circle1, circle2);
            if (shortest_segment.start.on(arc)) {
                return [dist, shortest_segment];
            } else {
                let dist_and_segment = [];

                dist_and_segment.push(Distance$1.point2circle(arc.start, circle2));
                dist_and_segment.push(Distance$1.point2circle(arc.end, circle2));
        ...
        static arc2arc(arc1, arc2) {
            ...
            if (shortest_segment.start.on(arc1) && shortest_segment.end.on(arc2)) {
                ...
                [dist_tmp, segment_tmp] = Distance$1.point2arc(arc1.start, arc2);
                if (segment_tmp.end.on(arc2)) {
                    dist_and_segment.push([dist_tmp, segment_tmp]);
                }

                [dist_tmp, segment_tmp] = Distance$1.point2arc(arc1.end, arc2);
                if (segment_tmp.end.on(arc2)) {
                    dist_and_segment.push([dist_tmp, segment_tmp]);
                }

                [dist_tmp, segment_tmp] = Distance$1.point2arc(arc2.start, arc1);
                if (segment_tmp.end.on(arc1)) {
                    dist_and_segment.push([dist_tmp, segment_tmp.reverse()]);
                }

                [dist_tmp, segment_tmp] = Distance$1.point2arc(arc2.end, arc1);
                if (segment_tmp.end.on(arc1)) {
                    dist_and_segment.push([dist_tmp, segment_tmp.reverse()]);
                }

                [dist_tmp, segment_tmp] = Distance$1.point2point(arc1.start, arc2.start);
                dist_and_segment.push([dist_tmp, segment_tmp]);

                [dist_tmp, segment_tmp] = Distance$1.point2point(arc1.start, arc2.end);
                dist_and_segment.push([dist_tmp, segment_tmp]);

                [dist_tmp, segment_tmp] = Distance$1.point2point(arc1.end, arc2.start);
                dist_and_segment.push([dist_tmp, segment_tmp]);

                [dist_tmp, segment_tmp] = Distance$1.point2point(arc1.end, arc2.end);
                dist_and_segment.push([dist_tmp, segment_tmp]);
        ...
        static point2polygon(point, polygon) {
            ...
            for (let edge of polygon.edges) {
                let [dist, shortest_segment] = (edge.shape instanceof Flatten$1.Segment) ?
                    Distance$1.point2segment(point, edge.shape) : Distance$1.point2arc(point, edge.shape);
        ...
        static minmax_tree_process_level(shape, level, min_stop, tree) {
            ...
            for (let node of level) {
                ...
                // Estimate min-max dist to the shape stored in the node.item, using node.item.key which is shape's box
                [mindist, maxdist] = Distance$1.box2box_minmax(shape.box, node.item.key);
            ...
            let new_level = [...new_level_left, ...new_level_right].filter(node => {
                // Node subtree quick reject, node.max is a subtree box
                let [mindist, maxdist] = Distance$1.box2box_minmax(shape.box, node.max);
                return (Flatten$1.Utils.LE(mindist, min_stop));
            });

            min_stop = Distance$1.minmax_tree_process_level(shape, new_level, min_stop, tree);
            return min_stop;
        }
         ...
         */
        static minmax_tree(shape, set, min_stop) {
            let tree = new IntervalTree$1();
            let level = [set.index.root];
            let squared_min_stop = min_stop < Number.POSITIVE_INFINITY ? min_stop * min_stop : Number.POSITIVE_INFINITY;
            squared_min_stop = Distance$1.minmax_tree_process_level(shape, level, squared_min_stop, tree);
            return tree;
        }
        ...
        static minmax_tree_calc_distance(shape, node, min_dist_and_segment) {
            let min_dist_and_segment_new, stop;
            if (node != null && !node.isNil()) {
                [min_dist_and_segment_new, stop] = Distance$1.minmax_tree_calc_distance(shape, node.left, min_dist_and_segment);
                ...
                let [dist, shortest_segment] = Distance$1.distance(shape, node.item.value);
                // console.log(dist)
                if (Flatten$1.Utils.LT(dist, min_dist_and_segment_new[0])) {
                    min_dist_and_segment_new = [dist, shortest_segment];
                }

                [min_dist_and_segment_new, stop] = Distance$1.minmax_tree_calc_distance(shape, node.right, min_dist_and_segment_new);
        ...
        static shape2planarSet(shape, set, min_stop = Number.POSITIVE_INFINITY) {
            ...
            if (set instanceof Flatten$1.PlanarSet) {
                let tree = Distance$1.minmax_tree(shape, set, min_stop);
                [min_dist_and_segment, stop] = Distance$1.minmax_tree_calc_distance(shape, tree.root, min_dist_and_segment);
            }
            return min_dist_and_segment;
        }
    ...
    function addToIntPoints(edge, pt, int_points)
    {
        ...
        if (Utils$1.EQ(len, edge.shape.length)) {
    ...
    function compareFn(ip1, ip2)
    {
        ...
        if (Utils$1.LT(ip1.arc_length, ip2.arc_length)) {
            return -1;
        }
        if (Utils$1.GT(ip1.arc_length, ip2.arc_length)) {
            return 1;
        }
        return 0;
    }
    ...
    function filterDuplicatedIntersections(intersections)
    {
        ...
        for (let i = 1; i < intersections.int_points1_sorted.length; i++) {
            ...
            if (!Utils$1.EQ(int_point_cur1.arc_length, int_point_ref1.arc_length)) {
        ...
        for (let i = 1; i < intersections.int_points2_sorted.length; i++) {
            ...
            if (int_point_ref2.id == -1 || /* can't be reference if already deleted */
                !(Utils$1.EQ(int_point_cur2.arc_length, int_point_ref2.arc_length))) {
